...
export default async function (ctx, inject) {
  ...
  ctx.beforeNuxtRender(() => {
    if (options.key) {
      const mapsScript = ctx.app.head.script.find((s) => s.hid == options.scriptId);
      console.log("mapsScript: ", ctx.app.head.script);
      mapsScript.src = `${options.scriptURL}?key=${options.key}&libraries=${librariesStr}&callback=initMap`;
    }
  });
